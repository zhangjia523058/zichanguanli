/**
 * @file Describe the file Created by Administrator on 2016/1/5.
...
(function () {
    ...
    var V; var A; var S; var G; var c; var X = navigator.appName;
    if (String(X) === 'Microsoft Internet Explorer') {
        S = true;
    }
    else if (String(X) === 'Opera') {
        c = true;
    }
    else {
        G = true;
    ...
    if ($.$crossFrame) {
        try {
            while (V.parent !== V && Number(V.parent[N][C]('frameset').length) === 1) {
                V = V.parent;
            }
        } catch (O) {
        }
        ...
        var A = {
            ...
            show: function () {
                ...
                for (var B = 0; B < A.length; B++) {
                    var _ = parseInt(A[B].style.zIndex, 10);
                    if (_ > $) {
                        $ = _;
                    }
                }
    ...
    function W(G, F) {
        if (G.getBoundingClientRect) {
            ...
            var C = G.offsetParent;
            if (C !== G) {
                while (C) {
                    H += C.offsetLeft;
                    _ += C.offsetTop;
                    if (String(R(C, 'position').toLowerCase()) === 'fixed') {
                        E = true;
                    }
                    else if (String(C.tagName.toLowerCase()) === 'body') {
            ...
            while (C.tagName && !A.ROOT_TAG.test(C.tagName)) {
                if (C.scrollTop || C.scrollLeft) {
                    if (!A.OP_SCROLL.test(P(C))) {
                        if (String(!c || C.style.overflow) !== 'visible') {
                            H -= C.scrollLeft;
                            _ -= C.scrollTop;
                        }
                    }
    ...
    function M($) {
        $ = $ || V;
        var B = $[N];
        var A = ($.innerWidth) ? $.innerWidth
               : (B[H] && B[H].clientWidth) ? B[H].clientWidth
                       : B.body.offsetWidth;
        var _ = ($.innerHeight) ? $.innerHeight
    ...
    function D($) {
        try {
            ...
            if ($dp.cal && !$dp.eCont && $dp.dd && _ !== $dp.el
                    && String($dp.dd.style.display) === 'block') {
                $dp.cal.close();
            }
        } catch ($) {
        }
    ...
    function U(K, C) {
        ...
        for (H in $) {
            if (String(H.substring(0, 1)) !== '$' && L[H] === undefined) {
                L[H] = $[H];
            }
        }
        if (C) {
            if (!J()) {
                _ = _ || setInterval(function () {
                    if (String(V[N].readyState) === 'complete') {
                        clearInterval(_);
                    }
                    U(null, true);
                }, 50);
            ...
            if (!L.el
                    || L.el['My97Mark'] === true
                    || L.el.disabled
                    || ($dp.dd && String(P($dp.dd)) !== 'none' && String($dp.dd.style.left) !== '-970px')) {
                try {
                    if (L.el['My97Mark']) {
                        L.el['My97Mark'] = false;
                    }
            ...
            if (F && Number(L.el.nodeType) === 1 && !a(L.el.initcfg, K)) {
                $dp.unbind(L.el);
                E(L.el, String(F.type) === 'focus' ? 'onclick'
                        : 'onfocus', function () {
                            U(K);
                        }
                );
        ...
        function J() {
            if (S && V !== Y && String(V[N].readyState) !== 'complete') {
                return false;
            }
            return true;
        }
    ...
    function R(_, $) {
        return _.currentStyle ? _.currentStyle[$]
    ...
    function I(G, _) {
        var D = G.el ? G.el.nodeName
                : 'INPUT';
        if (_ || G.eCont || new RegExp(/input|textarea|div|span|p|a/ig).test(D)) {
            G.elProp = String(D) === 'INPUT' ? 'value'
                    : 'innerHTML';
        }
        else {
            return;
        }
        if (String(G.lang) === 'auto') {
        ...
        function E(K, J) {
            var I = V[N].domain;
            var F = false;
            var G = '<iframe hideFocus="true" width="9" height="7" '
                + 'frameborder="0" border="0" scrolling="no" src="about:blank"></iframe>';
            K.innerHTML = G;
            var _ = $.$langList;
            var D = $.$skinList;
            var H;
            ...
            function C() {
                ...
                var $ = [
                        '<head><script>',
                        '',
                        'var doc=document, $d, $dp, $cfg=doc.cfg, $pdp = parent.$dp,'
                        + '$dt, $tdt, $sdt, $lastInput, $IE=$pdp.ie, $FF = $pdp.ff,$OPERA=$pdp.opera,'
                        + '$ny, $cMark = false;',
                        'if($cfg.eCont){$dp = {};for(var p in $pdp)$dp[p]=$pdp[p];}else{$dp=$pdp;};'
                        + 'for(var p in $cfg){$dp[p]=$cfg[p];}',
                        'doc.oncontextmenu=function(){try{$c._fillQS(!$dp.has.d,0);'
                        + 'showB($d.qsDivSel);}catch(e){};return false;};',
                ...
                $
                        .push('<script>var t;t=t||setInterval(function(){if(doc.ready){new My97DP();$cfg.onload();'
                                + '$c.autoSize();$cfg.setPos($dp);clearInterval(t);}},20);</script>');
                J.setPos = B;
                J.onload = Z;
                H.write('<html>');
                H.cfg = J;
        ...
        function B(J) {
            ...
            var C = J.position.top;
            var D = J.el;
            if (D === T) {
                return;
            }
            if (D !== J.srcEl && String((P(D)) === 'none' || String(D.type) === 'hidden')) {
                D = J.srcEl;
            }
            var I = W(D);
            var $ = F(Y);
            var E = M(V);
            var B = b(V);
            var G = $dp.dd.offsetHeight;
            var A = $dp.dd.offsetWidth;
            if (isNaN(C)) {
                C = 0;
            }
